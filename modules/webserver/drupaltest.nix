...
in
{
  options.webserver.drupaltest = {
    enable = lib.mkEnableOption "drupal test";
  };
  config = lib.mkIf config.webserver.drupaltest.enable {
    services = {
      nginx = {
        upstreams."php-${app}" = {
          servers = {
          ...
          "drupaltest.haztecaso.com" = {
            ...
            locations = {
              ...
              "~ ^/sites/.*/private/".extraConfig = ''
                return 403;
              '';
              "location ~ ^/sites/[^/]+/files/.*\.php$".extraConfig = ''
                deny all;
              '';
